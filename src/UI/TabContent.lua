...
local ARCHON_TAB_CONFIG = {
	sections = {
		...
		},
		{
			...
			name = "Sporefall",
			dropdownInitializer = "InitializeArchonSporefallDropdown",
			editBoxPrefix = "archonSporefall",
			source = "archon",
			category = "sporefall",
			divider = true
		}
...
-- Configuration for Wowhead tab sections
local WOWHEAD_TAB_CONFIG = {
	sections = {
		{
			name = "Mythic+",
...
function TabContent.CreateArchonTab(dialog, tab)
	CreateSourceTab(dialog, tab, ARCHON_TAB_CONFIG, "archon")
end

function TabContent.CreateWowheadTab(dialog, tab)
	CreateSourceTab(dialog, tab, WOWHEAD_TAB_CONFIG, "wowhead")
end
